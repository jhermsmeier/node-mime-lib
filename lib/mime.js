...
MIME.prototype = {
  ...
  decodeQP: function( input, charset, wordMode ) {
    ...
    return /^(ascii|utf-?8|utf-?16-?le|ucs-?2)$/i.test( charset ) ?
      buffer.toString( charset.replace( /-/g, '' ) ) :
      MIME.Iconv.decode( buffer, charset )
  ...
  decodeWord: function( input ) {
    ...
    return input.replace(
      ...
      function( match, charset, type, data ) {
        ...
        return type === 'B' ?
          self.decodeBase64( data, charset ) :
          self.decodeQP( data, charset, true )
